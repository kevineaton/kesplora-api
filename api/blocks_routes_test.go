...
func (suite SuiteTestsBlocksRoutes) TestSimpleBlocksAdminRoutesCRUD() {
	...
	code, res, err := testEndpoint(http.MethodPost, "/blocks/text", b, routeCreateBlock, admin.Access)
	suite.Nil(err)
	require.Equal(http.StatusBadRequest, code, res)
	code, res, err = testEndpoint(http.MethodPost, "/blocks/mooo", b, routeCreateBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusBadRequest, code, res)
	code, res, err = testEndpoint(http.MethodPost, "/blocks/text", b, routeCreateBlock, user.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockText.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPost, "/blocks/external", b, routeCreateBlock, admin.Access)
	suite.Nil(err)
	require.Equal(http.StatusCreated, code, res)
	blockExternal := &Block{}
	m, err = testEndpointResultToMap(res)
	...
	defer DeleteBlock(blockText.ID)
	defer handleBlockDelete(blockText.BlockType, blockText.ID)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	m, err = testEndpointResultToMap(res)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPost, "/blocks/presentation", b, routeCreateBlock, admin.Access)
	suite.Nil(err)
	require.Equal(http.StatusCreated, code, res)
	...
	defer DeleteBlock(blockText.ID)
	defer handleBlockDelete(blockText.BlockType, blockText.ID)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockPresentation.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	m, err = testEndpointResultToMap(res)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPatch, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeUpdateBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPatch, fmt.Sprintf("/blocks/%d", blockText.ID), b, routeUpdateBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockText.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPatch, fmt.Sprintf("/blocks/%d", blockPresentation.ID), b, routeUpdateBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockPresentation.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	// get for the site
	code, res, err = testEndpoint(http.MethodGet, "/blocks", b, routeGetBlocksOnSite, user.Access)
	suite.Nil(err)
	suite.Equal(http.StatusForbidden, code, res)
	code, res, err = testEndpoint(http.MethodGet, "/blocks", b, routeGetBlocksOnSite, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodPut, fmt.Sprintf("/modules/%d/blocks/%d/order/1", module1.ID, blockExternal.ID), b, routeLinkBlockAndModule, user.Access)
	suite.Nil(err)
	suite.Equal(http.StatusForbidden, code, res)
	code, res, err = testEndpoint(http.MethodPut, fmt.Sprintf("/modules/%d/blocks/%d/order/1", module1.ID, blockExternal.ID), b, routeLinkBlockAndModule, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodPut, fmt.Sprintf("/modules/%d/blocks/%d/order/2", module1.ID, blockText.ID), b, routeLinkBlockAndModule, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)

	// get for module, make sure presentation isn't there
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/modules/%d/blocks", module1.ID), b, routeGetBlocksForModule, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	s, err = testEndpointResultToSlice(res)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/modules/%d/blocks/%d", module1.ID, blockExternal.ID), b, routeUnlinkBlockAndModule, user.Access)
	suite.Nil(err)
	suite.Equal(http.StatusForbidden, code, res)
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/modules/%d/blocks/%d", module1.ID, blockExternal.ID), b, routeUnlinkBlockAndModule, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/modules/%d/blocks", module1.ID), b, routeGetBlocksForModule, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeDeleteBlock, user.Access)
	suite.Nil(err)
	suite.Equal(http.StatusForbidden, code, res)
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeDeleteBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockExternal.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/blocks/%d", blockPresentation.ID), b, routeDeleteBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockPresentation.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
	...
	code, res, err = testEndpoint(http.MethodDelete, fmt.Sprintf("/blocks/%d", blockText.ID), b, routeDeleteBlock, admin.Access)
	suite.Nil(err)
	suite.Equal(http.StatusOK, code, res)
	code, res, err = testEndpoint(http.MethodGet, fmt.Sprintf("/blocks/%d", blockText.ID), b, routeGetBlock, admin.Access)
	suite.Nil(err)
